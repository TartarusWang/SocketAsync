...
namespace SocketAsyncServer
{
    ...
    internal sealed class Token : IDisposable
    {
        ...
        internal void SetData(SocketAsyncEventArgs args)
        {
            ...
            {
                throw new ArgumentOutOfRangeException("count", string.Format(CultureInfo.CurrentCulture, "Adding {0} bytes on buffer which has {1} bytes, the listener buffer will overflow.", count, this.currentIndex));
            }
